...
class PositionSearchProblem(search.SearchProblem):
    ...
    def __init__(self, gameState, costFn = lambda x: 1, goal=0, start=None, warn=True):
        """
        Stores the start and goal.

        gameState: A GameState object (pacman.py)
        costFn: A function from a search state (tuple) to a non-negative number
        goal: num strawberries covered with greenhouses
        """
        self.walls = gameState.getWalls()
        self.startState = gameState.getGHworkerPosition()
        if start != None: self.startState = start
        self.goal = gameState.getNumFood() #num strawberries to cover with gh's
        self.numGHs = gameState.getNumGHs() 
        self.costFn = costFn
...
class FoodSearchProblem:
    ...
    """
    def __init__(self, startingGameState):
        self.start = (startingGameState.getGHworkerPosition(), startingGameState.getFood())
        self.walls = startingGameState.getWalls()
